...
seenNewData = [{} for i in tables2]

for j in table:
    for c, i in enumerate(tables2):
        toadd = []
        for ii in i.split(','):
            toadd.append(j[d0.index(ii)])
        if str(toadd) not in seenNewData[c]:
            newData[c].append(toadd)
            seenNewData[c][str(toadd)] = True

uniqueMap = []
for i in newData:
    seenStuff = [{} for j in range(len(i[0]))]
